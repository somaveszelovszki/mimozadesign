...
const isRouteActive = (pathname: string, href: string): boolean => pathname === href || pathname.startsWith(`${href}/`)
    ...
    <NavigationMenu viewport={false} className={className}>
      <NavigationMenuList className='flex-wrap justify-start gap-0'>
        {navigationData.map(navItem => {
          const divider = navItem.dividerBefore ? (
            <li
              key={`${navItem.title}-divider`}
              role='separator'
              aria-orientation='vertical'
              className='mx-3 h-7 self-center border-l-2 border-black'
            />
          ) : null
              ...
              <Fragment key={navItem.title}>
                ...
                <NavigationMenuItem>
                  <NavigationMenuLink
                    ...
                    className={cn(
                      navigationMenuTriggerStyle(),
                      "relative rounded-full bg-transparent px-3 py-1.5 text-base! font-normal transition-colors duration-200 after:absolute after:bottom-1.5 after:left-1/2 after:h-0.5 after:w-0 after:-translate-x-1/2 after:bg-current after:transition-[width] after:duration-[400ms] after:content-['']",
          ...
          return (
            <Fragment key={navItem.title}>
              {divider}
              <NavigationMenuItem>
                <NavigationMenuTrigger className='hover:text-primary hover:bg-primary/5 dark:hover:bg-primary/10 focus:text-primary focus:bg-primary/5 dark:focus:bg-primary/10 data-[state=open]:text-primary data-[state=open]:bg-primary/5 dark:data-[state=open]:bg-primary/10 data-[state=open]:hover:bg-primary/5 dark:data-[state=open]:hover:bg-primary/10 bg-transparent px-3 py-1.5 text-base text-black [&>svg]:size-4'>
                  {navItem.title}
                </NavigationMenuTrigger>
